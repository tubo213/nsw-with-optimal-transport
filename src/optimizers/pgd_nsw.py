...
def compute_pi_pgd_nsw(
    ...
    n_rank = expo.shape[0]
    rel_mat: torch.Tensor = torch.FloatTensor(rel_mat).to(device).double()  # (n_query, n_doc)
    expo: torch.Tensor = torch.FloatTensor(expo).to(device).double()  # (K, 1)
    ...
    # アイテムからの供給量
    a: torch.Tensor = torch.DoubleTensor(high).view(1, -1, 1).to(device)
    # ダミー列への輸送量はアイテム数 - 表示数
    dummy_demand = a.sum(dim=1) - n_rank  # (n_query, 1)
    b = torch.ones(n_query, n_rank + 1, 1, device=device, dtype=torch.double)
    b[:, -1, :] = dummy_demand

    # 初期値は一様分布
    # 表示数 < アイテム数の場合にはダミー列に輸送されるようにする
    X = nn.Parameter(torch.rand(n_query, n_doc, n_rank + 1).to(device).double())
    X.data = sinkhorn(X.data, a, b, n_iter=ot_n_iter, eps=eps, apply_kernel=False)
    ...
    history.append_pi_if_not_full(iter=-1, pi=X.data[0].clone().detach().cpu().numpy())

    for iter in tqdm(range(max_iter), leave=False):
        optimier.zero_grad()
        with autocast(enabled=use_amp):
            # 損失を計算
            loss = compute_nsw_loss(X[:, :, :-1], click_prob, am_rel, eps=1e-6)
        ...
        # 勾配を記録
        history.append_grad_if_not_full(iter, -X.grad[0].clone().detach().cpu().numpy())  # type: ignore
        ...
        with torch.no_grad():
            if apply_negative_to_X_bf_sa:
                X.data = -X.data
            X.data = sinkhorn(
                (X - X.min() + eps), a, b, n_iter=ot_n_iter, eps=eps, apply_kernel=apply_kernel
            )
        ...
        history.append_loss(iter, loss.item())
        history.append_grad_norm(iter, grad_norm.item())
        history.append_pi_if_not_full(iter, X[0].detach().cpu().numpy())
